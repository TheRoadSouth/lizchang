/**
 * Racehorse (Rh) Modal jQuery Modal Plugin
 * @version 1.1
...
;(function($) {
	...
	var RhModal = {
		...
		options: {
			...
			minHeight: 50,
			closeButton: true,
			closeButtonTemplate: '<div id="RhCloseWrap"><a id="RhClose">X</a></div>',
		...
		show: function() {
			...
			if (this.options.href) {
				$(this.options.href).css({'display' : 'block'});
			}
		...
		animate: function(contentHeight) {
            ...
            if (contentHeight !== false) {
				...
				if (this.options.marginTop > 0 && !this.options.scrollTop) {
					...
					$modal.css({'margin-top' : scrollPosition + this.options.marginTop});
				}
		...
		remove: function() {
			...
			if (this.options.href && this.options.href !== '') {
				this.unwrapContent();
				$(this.options.href).css({'display' : 'none'});
				$('#RhCloseWrap').remove();
			} else {
				$('#RhModal').remove();
			}
